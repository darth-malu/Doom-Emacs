...
-{.*} - return all without the match {.*} - so return all with no tags

tags-todo - sysmbol that search for tags specified

You can modify variables inside the agenda config *(())* after the tags-todo "Stuff"
        TODO Check if other emacs vars outside agendas work
